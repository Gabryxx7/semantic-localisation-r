...
## ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
library(tictoc)
run_trace <- function(test_trace){
  n_rooms <- nrow(test_trace$data)
  n_timeslots <- ncol(test_trace$data)-2 # Remove tSId and tag
  cat(paste0("\n************ Running Trace ", test_trace$idx, "( ", n_rooms, " x ", n_timeslots, " ) ************\n"))
  tic("Trace_Test")
  ...
  res_trace <- start_trace(.GlobalEnv[['test_trace_data']]) %>%
    ...
    add_state(agents(set(a4)), rooms(set(,any))) %~~>% # non-fuzzy
    end_trace()
  timing_toc <- toc()
  ...
  return(list(out=res_trace, occ=occupancy, mean_occ=mean_occ, timing=timing_toc, n_rooms=n_rooms, n_timeslots=n_timeslots))
}


## ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
test_traces <- invisible(generate_test_traces(5,6))
test_output <- list()
for(i in 1:nrow(test_traces$configs)){
  trace_to_test <- test_traces$traces[[i]]
  test_output <- append(test_output, list(run_trace(list(data=test_traces$traces[[i]], idx=i))))
}
test_output_filtered <- lapply(test_output, function(x) list(time=x$timing$toc[['elapsed']], mean_occ=x$mean_occ, n_rooms=x$n_rooms, n_timeslots=x$n_timeslots))
test_output_df <- data.frame(apply(do.call("rbind", test_output_filtered), 2, unlist))
test_output_df$time_s <- test_output_df$time/1000000


## ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
scalability_plot <- ggplot(data=test_output_df, aes(x=n_timeslots, y=time_s, fill=n_rooms, color=n_rooms)) +
  geom_line() +
  xlab("No. of Rooms") +
